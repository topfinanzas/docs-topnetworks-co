    ...
    <div className="prose prose-brand max-w-none">
      <ReactMarkdown
        ...
        components={{
          ...
          h2: ({ node: _node, ...props }) => (
            <h2
              className="text-2xl font-semibold text-gray-800 mt-10 mb-4 pb-2 border-b border-gray-200"
              {...props}
            />
          ),
          h3: ({ node: _node, ...props }) => (
            <h3
              className="text-xl font-medium text-gray-800 mt-8 mb-3"
              {...props}
            />
          ),
          p: ({ node: _node, ...props }) => (
          ...
          ul: ({ node: _node, ...props }) => (
            <ul
              className="list-disc pl-6 mb-4 space-y-2 text-gray-600"
              {...props}
            />
          ),
          ol: ({ node: _node, ...props }) => (
            <ol
              className="list-decimal pl-6 mb-4 space-y-2 text-gray-600"
              {...props}
            />
          ),
          ...
          a: ({ node: _node, href, ...props }) => {
            ...
            return (
              <a
                href={targetHref}
                className="text-brand-blue-600 hover:text-brand-blue-800 underline transition-colors"
                {...props}
              />
            );
          },
          code: ({ node: _node, className, children, ...props }) => {
            ...
            if (
              ...
              /^#([0-9A-Fa-f]{3}){1,2}$/.test(children)
            ) {
              return (
                <code
                  className="bg-gray-100 text-brand-blue-700 px-1.5 py-0.5 rounded text-sm font-mono inline-flex items-center gap-1.5"
                  {...props}
                >
                  <span
            ...
            return isInline ? (
              <code
                className="bg-gray-100 text-brand-blue-700 px-1.5 py-0.5 rounded text-sm font-mono"
                {...props}
              >
                {children}
              </code>
          ...
          blockquote: ({ node: _node, ...props }) => (
            <blockquote
              className="border-l-4 border-brand-cyan-500 bg-brand-cyan-50/50 pl-4 py-2 pr-4 italic text-gray-700 my-4 rounded-r-lg"
              {...props}
            />
          ),
