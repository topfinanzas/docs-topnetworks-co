...
export default function Layout() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-white text-gray-800 font-sans">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 transform border-r border-gray-200 bg-gray-50 transition-transform duration-200 ease-in-out lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white min-h-16">
          <div className="flex items-center w-full">
            <img
              src="/media/topnetworks-positivo-sinfondo.webp"
              alt="TopNetworks"
              className="w-full max-w-[180px] h-auto object-contain my-2"
            />
          </div>
          <button
            onClick={() => setSidebarOpen(false)}
